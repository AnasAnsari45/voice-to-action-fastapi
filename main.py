...
# ✅ Correct HF model for extraction
...
def download_media_file(media_url, filename="voice_note.ogg"):
    """Download the voice message to local storage"""
    headers = {"Authorization": f"Bearer {WABA_TOKEN}"}
...
def transcribe_with_whisper(file_path):
    """Transcribe multilingual voice message using Whisper API"""
    try:
...
def structure_transcription(text):
    """Send transcription to HF LLM and extract structured JSON"""
    prompt = f"""
    ...
    try:
        ...
        if messages:
            ...
            if msg_type == "text":
                ...
                # Step 4: Structure using Hugging Face model
                if transcription:
